...
class MainView: NSView, URLSessionDataDelegate, URLSessionDelegate, URLSessionDownloadDelegate {
    ...
    @IBOutlet var EntPlistTextField: NSTextField!
...
//    let plistbuddyPath = "/usr/libexec/plistbuddy"
    ...
    @objc func signingThread(){
        ...
        DispatchQueue.main.sync {
            ...
            entPlistPath = self.EntPlistTextField.stringValue.trimmingCharacters(in: CharacterSet.whitespacesAndNewlines)
            ...
            for file in files {
                ...
                if provisioningFile != nil || useAppBundleProfile {
                    ...
                    if let profile = ProvisioningProfile(filename: useAppBundleProfile ? appBundleProvisioningFilePath : provisioningFile!, skipGetTaskAllow: shouldSkipGetTaskAllow){
                        if entPlistPath.lengthOfBytes(using: String.Encoding.utf8)>0
                        {
                            do{
                                try FileManager.default.copyItem(atPath: entPlistPath, toPath:entitlementsPlist)
